...
import com.johndev.aitrainer.Iterations.IterationsMethodActivity.Companion.printDataViewModel
...
class PrintDatasetFragment : Fragment() {
    ...
    private fun setupObservers() {
        printDataViewModel.getResultSaved().observe(viewLifecycleOwner) { isSuccess ->
            if (isSuccess == null) {
    ...
    private fun configureExport(nameFile: String, extensionFile: String, resultJson: MutableList<ResultsJSON>) {
        when (extensionFile) {
            getString(R.string.file_extension_json) -> { printDataViewModel.createJSON(nameFile, extensionFile, resultJson) }
            getString(R.string.file_extension_txt) -> { printDataViewModel.createTXT(nameFile, extensionFile, resultJson) }
            getString(R.string.file_extension_csv) -> { printDataViewModel.createCSV(nameFile, extensionFile, resultJson) }
        }
    }
