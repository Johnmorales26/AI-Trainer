...
class AutomaticChargeDatasetsFragment : Fragment(), OnDatasetListener {
    ...
    private fun setupObservers() {
        automaticChargeDataViewModel.getTwoDimensionalResult().observe(viewLifecycleOwner) { dataset ->
            if (dataset == null) {
        ...
        if (requestCode == pickPdfFile
            ...
            resultData?.data?.also { uri ->
                lifecycleScope.launch {
                    automaticChargeDataViewModel.readTextFromUri(uri, requireContext())
                }
            }
        }
    }
