...
class AutomaticPrintDataMemory() {

    suspend fun createJSON(name: String, extension: String,resultList: MutableList<ResultsAutomaticJSON>): Boolean {
        val gson = Gson()
    ...
    suspend fun createTXT(name: String, extension: String,resultList: MutableList<ResultsAutomaticJSON>): Boolean {
        var exportTXT = ""
        resultList.forEach {
            exportTXT += "Iteraciones: ${it.id} ---------- W: ${it.w0} ---------- W: ${it.w1} ---------- JW: ${it.jw}\n"
        }
        return writeToFile(name,  extension, exportTXT)
    }

    suspend fun createCSV(name: String, extension: String,resultList: MutableList<ResultsAutomaticJSON>): Boolean {
        var exportCSV = ""
        resultList.forEach {
            exportCSV += "${it.id}$SEPARATOR_CSV${it.w0}$SEPARATOR_CSV${it.w1}$SEPARATOR_CSV${it.jw}\n"
        }
