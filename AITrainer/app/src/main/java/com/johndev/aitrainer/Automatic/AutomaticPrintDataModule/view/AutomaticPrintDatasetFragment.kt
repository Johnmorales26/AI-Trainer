...
import com.johndev.aitrainer.Automatic.AutomaticActivity.Companion.automaticPrintViewModel
...
class AutomaticPrintDatasetFragment : Fragment() {
    ...
    private val resultList: MutableList<ResultsAutomaticJSON> = ArrayList()
    ...
    private fun setupObservers() {
        automaticPrintViewModel.getResultSaved().observe(viewLifecycleOwner) { isSuccess ->
            if (isSuccess == null) {
        ...
        automaticOpViewModel.getResultAutomatic().observe(viewLifecycleOwner) { result ->
            if (result == null) {
                ...
                var i = 0
                while (i < result.size){
                    resultList.add(ResultsAutomaticJSON(result[i].id,result[i].W0, result[i].W1, result[i].J))
                    i++
                }
            }
        }
    ...
    private fun configureExport(nameFile: String, extensionFile: String, resultJson: MutableList<ResultsAutomaticJSON>) {
        when (extensionFile) {
            getString(R.string.file_extension_json) -> { automaticPrintViewModel.createJSON(nameFile, extensionFile, resultJson) }
            getString(R.string.file_extension_txt) -> { automaticPrintViewModel.createTXT(nameFile, extensionFile, resultJson) }
            getString(R.string.file_extension_csv) -> { automaticPrintViewModel.createCSV(nameFile, extensionFile, resultJson) }
        }
    }
